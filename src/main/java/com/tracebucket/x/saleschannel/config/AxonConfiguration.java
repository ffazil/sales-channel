...
//import com.tracebucket.x.saleschannel.domain.model.Task;
...
//		commandBus.setTransactionManager(new SpringTransactionManager(transactionManager));
	...
	public EventSourcingRepository<SalesChannel> salesChannelRepository() {
		...
		EventSourcingRepository<SalesChannel> repository = new EventSourcingRepository<SalesChannel>(SalesChannel.class, eventStore);
		repository.setEventBus(eventBus());
		return repository;
	}
	
	@Bean
	public AggregateAnnotationCommandHandler<SalesChannel> salesChannelCommandHandler() {
		AggregateAnnotationCommandHandler<SalesChannel> commandHandler = AggregateAnnotationCommandHandler.subscribe(SalesChannel.class, salesChannelRepository(), commandBus());
